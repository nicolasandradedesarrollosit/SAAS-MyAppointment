import { createBusinessModel } from "../models/modelPostBusiness.js";

export default async function createBusinessController(req, res) {
  try {
    const result = validateFields(req.body);

    if (!result.ok) return res.status(400).json({ ok: false, errors: result.errors })

    const created = await createBusinessModel(result.value);
