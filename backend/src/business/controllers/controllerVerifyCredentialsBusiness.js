import { verifyCredentialsBusinessModel } from '../models/modelVerifyCredentialsBusiness.js';
import jwt from 'jsonwebtoken';

export default async function controllerUserVerifyCredentials(req, res) {
  try {
    const result = validateFields(req.body);

    if (!result.ok) return res.status(400).json({ ok: false, errors: result.errors });
    const tokenCredentials = await verifyCredentialsBusinessModel(result.value);
